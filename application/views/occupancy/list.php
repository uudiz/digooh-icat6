<table class="table-list"  width="100%" border="0">
		...
		?>
			...
			<tr <?php if($index%2 != 0):?>class="even" onmouseout="this.className='even'" <?php else:?>onmouseout="this.className=''"<?php endif;?>  onmouseover="this.className='onSelected'">
			  <td>
			  	<?php if(mb_strlen($row->name) > 64){echo mb_substr($row->name, 0, 64).'..';}else{echo $row->name;}?>
			  </td>
			  <td>
			  ...
			  <td>
			  	<?php if($auth > 1):?>
			  		<a href="/criteria/edit?id=<?php echo $row->id;?>&width=450&height=320"  class="thickbox" title="<?php echo lang('edit.criteria');?>"><img id="edit_<?php echo $row->id;?>" src="/images/icons/24-edit.png"  title="<?php echo lang('edit');?>" /></a>
