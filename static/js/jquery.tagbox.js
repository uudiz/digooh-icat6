...
(function(jQuery) {
  jQuery.fn.tagBox = function(options) {
    var defaults = {
      ...
      tagButtonClassName: '',
      tagButtonTitle: 'Add',
      tagShowAddButton: true,
      confirmRemoval: false,
      confirmRemovalText: 'Are you sure to remove the tag？',
...
//  Hide Element
  ...
  //  Create DOM Elements
      ...
      */
    	  var $tagBox = jQuery('<span class="'+options.className+'-container"></span>').append($tag_input_elem).append($tag_list_elem);
  ...
  //  Generate Tags List from Input item
      ...
      if(!options.readonly) {
    	 $tag_input_elem.change(function() {
  ...
  //  Methods
     ...
     /*   for(var i = 0;i<tags_list.length;i++){
            $tag_input_elem.find('option[value="'+tags_list[i]+'"]').attr("disabled", "disabled");
        }
      */  
      ...
      function generate_tags_list(tags_list) {
        ...
        jQuery.each(tags_list, function(key, val) {
          if(val != "") {
            var remove_tag_link = (options.readonly||options.notShowRemove) ? '' : '<a href="javascript:void(0)" class="'+options.className+'-remove '+options.className+'-remove-'+uuid+'" title="Remove Tag" rel="'+val+'">'+options.removeTagText+'</a>';
