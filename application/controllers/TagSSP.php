...
class TagSSP extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->get_auth() <= 2) {
            ...
            $data['body_file'] = 'bootstrap/ssp_tags/index';
        }
    ...
    public function getTableData()
    {
        ...
        $rest = $this->device->get_ssptag_list($this->get_cid(), false, $offset, $limit, $order_item, $order, $name);
    ...
    public function edit()
    {
        ...
        $data['title'] = $this->lang->line('create.tag');

        if ($id) {
            $data['title'] = $this->lang->line('edit.tag');
            $region = $this->device->get_ssptag($id);
            if ($region) {
                $data['data'] = $region;
            } else {
        ...
        $data['body_file'] = 'bootstrap/ssp_tags/form';
        $this->load->view('bootstrap/layout/basiclayout', $data);
    }
    ...
    public function refresh($curpage = 1, $order_item = 'name', $order = 'asc', $main = false)
    {
        ...
        if ($main) {
            $data['body_view'] = 'org/tagssp/tag-list';
            $data['body_file'] = 'org/tagssp/tag';
            $this->load->view('include/main2', $data);
        } else {
            $this->load->view('org/tagssp/tag-list', $data);
        }
    }
    ...
    public function add()
    {
        ...
        $this->load->view('org/tagssp/add_tag', $data);
    }
    ...
    public function do_save()
    {
        ...
        if ($cid <= 0) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $flag = $this->device->get_ssptag_byname($id, $cid, $name);
                if ($id >= 0 && $flag) {
                    $result = array('code' => 1, 'msg' => sprintf($this->lang->line('tag.name.exsit'), $this->input->post('name')));
                } else {
                    if ($id > 0) {
                        $this->device->update_ssptag($data, $id);
                    } else {
                        $id = $this->device->add_ssptag($data, $this->get_uid());
                    }
                    if ($id !== false) {
                        $data['id'] = $id;
                        ...
                        if ($players) {
                            $this->device->sync_ssptag_players($players, $id);
                        }
                        $data['id'] = $id;
    ...
    public function do_delete()
    {
        ...
        $this->load->model('device');
        if ($this->device->delete_ssptag($gid)) {
            $result['code'] = 0;
    ...
    public function do_upload()
    {
        ...
        if (!$this->upload->do_upload('file')) {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                $flag = $this->device->get_ssptag_byname(0, $cid, $line);
                if ($flag) {
                    $skiped_cnt++;
                    continue;
                } else {
