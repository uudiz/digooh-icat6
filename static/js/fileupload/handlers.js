...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesSelected > 0) {
			var c = document.getElementById(this.customSettings.cancelButtonId);
			if(c != null){
				c.disabled = false;
			}
			
			if (this.customSettings.working()) {
...
function uploadSuccess(file, serverData) {
	try {
		...
		json = toJsonObj(serverData);
		if(json != null && json.code == 0 ){
			mediaLib.uploader.data.push(serverData);
		}
	} catch (ex) {
