...
class Campaign extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['body_file'] = 'bootstrap/campaigns/index';
    ...
    public function getTableData()
    {
        ...
        if ($this->get_auth() < 2) {
            $this->load->model('membership');
            $user = $this->membership->get_user($this->get_uid(), true);
            if (!$user->campaigns) {
                $data['total'] = 0;
                $data['rows'] = [];
    ...
    public function edit()
    {
        ...
        if ($auth == 0) {
            ...
            $data['can_replace_main'] = isset($user->can_replace_main) ? $user->can_replace_main : 0;
        } elseif ($auth >= 5) {
            $data['can_publish'] = 1;
            $data['can_replace_main']  = 1;
        } else {
            $data['can_publish'] = 0;
        }
        ...
        $data['nxslot'] = $company->nxslot;
        $data['shareblock'] = isset($company->shareblock) ? $company->shareblock : 1;
        if ($playlist_id) {
            //$playlist = $this->program->get_playlist($playlist_id);
            $playlist = $this->program->get_campaign($playlist_id, true);
            $playCnt = 1;
            switch ($playlist->play_cnt_type) {
                case 0:
                    $playCnt = $playlist->play_count;
                    break;
                case 1:
                    $playCnt = $playlist->play_weight;
                    break;
                case 2:
                    $playCnt = $playlist->play_total;
                    break;
                case 9:
                    $playCnt = $data['nxslot'];
                    break;
            }
            $playlist->play_count = $playCnt;

            $data['data'] = $playlist;
            $data['readonly'] = ($this->get_cid() != $playlist->company_id) ? 1 : 0;
            $media = $this->program->getPlaylistAreaMedia($playlist_id, 101);


            $data['media'] = json_encode($media);
            if ($playlist->priority == 8) {
                $players = $this->program->get_player_by_campaign($playlist->main_campaign_id, false);
                $data['players'] = $players;

                if ($playlist->main_campaign_id) {
                    $main_campaign = $this->program->get_campaign($playlist->main_campaign_id);
                    if ($main_campaign) {
                        $data['is_main_campaign_published'] = $main_campaign->published;
                    }
                }
            } else {
                $extended_cnt = $this->program->get_extended_campaigns_count($playlist_id);
                if ($extended_cnt['total'] > 0) {
                    $data['extended_cnt'] = $extended_cnt['total'];

                    if ($extended_cnt['replaced_extended_cnt'] > 0) {
                        $data['replaced_extended_cnt'] = $extended_cnt['replaced_extended_cnt'];
                    }
                }
            }
        }
        ...
        $data['main_campaigns'] = $this->program->get_main_campaigns($this->get_cid());

        $data['body_file'] = 'bootstrap/campaigns/form';
        $this->load->view('bootstrap/layout/basiclayout', $data);
    }
    ...
    public function getAffectedPlayers()
    {
        ...
        $campaign = $this->program->get_campaign($id);
    ...
    public function do_delete_check()
    {
        $id = $this->input->post('id');
        $this->load->model('program');
        $campaign = $this->program->get_campaign($id);
        $code = 0;
        $msg = $this->lang->line('tip.remove.item');
        if ($campaign) {
            if ($campaign->priority != 8) {
                $extended_campaigns_cnt = $this->program->get_extended_campaigns_count($id);
                if ($extended_campaigns_cnt['total'] > 0) {
                    $msg = $msg . "<br>" . $extended_campaigns_cnt['total'] . " extended campaigns will be deleted, too.";
                }
            }
        } else {
            $code = 1;
    ...
    public function do_delete()
    {
        ...
        if ($id) {
            ...
            if ($this->program->delete_playlist($id)) {
                ...
                $extended_campaings = $this->program->get_extended_campaigns($id);
                if ($extended_campaings) {
                    foreach ($extended_campaings as $extended_campaign) {
                        $this->program->real_delete_playlist($extended_campaign->id);
                    }
                }
            }
        }
    ...
    public function view_screen()
    {
        ...
        //删除临时数据
        $this->program->update_area_media_commit($id);
        ...
        $area_list = $this->program->get_area_list($template->id);
        foreach ($area_list as $area) {
            if ($area->area_type == $this->config->item('area_type_staticText')) {
                $staticText = $this->program->get_playlist_area_static_text_setting($id, $area->id);
        ...
        $data = $this->get_data();
        $data['body_file'] = 'program/campaign/view_screen';
        $data['playlist'] = $playlist;
        $data['template'] = $template;
        $data['area_list'] = $area_list;
        $data['content'] = $content;
        $data['html_content'] = $html_content;
        $abs_url = $this->program->get_playlist_template_preview_url($template->company_id, $id, true);

        $this->program->copy_playlist_template_preview_url($template, $id);
    ...
    public function view()
    {
        ...
        $data = $this->get_data();
        $data['body_file'] = 'program/campaign/view';
        $data['playlist'] = $playlist;
        $data['template'] = $template;
        $data['area_list'] = $area_list;

        $abs_url = $this->program->get_playlist_template_preview_url($template->company_id, $id, true);
        if (file_exists($abs_url)) {
            $data['preview_url'] = $this->program->get_playlist_template_preview_url($template->company_id, $id);
        } else {
            $this->program->copy_playlist_template_preview_url($template, $id);
            $data['preview_url'] = $template->preview_url;
        }
    ...
    public function do_save($publish = false)
    {
        ...
        if ($is_updating) {
            ...
            if ($deletes && !empty($deletes)) {
                $this->program->delete_media($deletes);
                $result['deletes'] = $deletes;
            }

            //delete deleted
            $this->program->delete_playlist_area_media_temp($playlist_id, $this->config->item('area_media_flag_delete'));

            /*
            if ($ids && !empty($ids)) {
                $this->program->update_media_flag($ids, $this->config->item('area_media_flag_ok'));
                //$result['ids'] = $ids;
            }
            */
        }
        ...
        if ($is_updating) {
            ...
            $extended_campaigns = $this->program->get_extended_campaigns($playlist_id);

            if ($extended_campaigns) {
                $extended_data = [];
                foreach ($data as $key => $val) {
                    if ($key == "play_cnt_type" || $key == "play_count" || $key == "booked" || $key == "play_weight" || $key == "play_total" || $key == 'start_date' || $key == 'end_date' || $key == "is_grouped" || $key == "time_flag" || $key == "start_timeH" || $key == "end_timeH") {
                        $extended_data[$key] = $val;
                    }
                }

                foreach ($extended_campaigns as $extended_campaign) {
                    // $extended_data['published'] = 0;
                    if ($publish == false && $pls->published == 1) {
                        if ($extended_campaign->published == 1) {
                            $this->program->reset_player_least_while_update_campaign($extended_campaign);
                            $this->program->delete_planed_records($extended_campaign->id, date("Y-m-d"));
                        }
                    }
                    if ($publish != true) {
                        $extended_data['published'] = 0;
                    }
                    $this->program->update_playlist($extended_data, $extended_campaign->id);
                }
            }
        } else {
        ...
        if (!empty($media)) {
            ...
            $this->program->delete_playlist_area_media_not_in_list($playlist_id, $area_id, $cur_media);


            foreach ($media as $medium) {
                //if $medium has area_media_id, update the cat_playlist_area_media table, else insert   
                $medium_array =  array(
                ...
                if (isset($medium->area_media_id)) {
                    $this->program->update_area_media(
                        $medium_array,
                        $medium->area_media_id
                    );
                    $valid_media[] = $medium->area_media_id;
                } else {
                    $id = $this->program->add_area_media(
                        $medium_array,
                        $uid
                    );
                }
                ...
                if ($medium->media_type == 2 && $medium->approved != 2) {
                    $hasVideo = true;
                }
        ...
        $this->sync_campaign($playlist_id, $priority);
        ...
        if ($players) {
            ...
            foreach ($players as $player) {
                $items[] = array('player_id' => $player->id, 'campaign_id' => $playlist_id);
        ...
        if ($is_updating) {
            if ($extended_campaigns) {
                $players_of_main = $this->program->get_sel_player_array_by_campaign($playlist_id);
                foreach ($extended_campaigns as $campaign) {
                    $players_of_campaign = $this->program->get_sel_player_array_by_campaign($campaign->id);
                    if ($players_of_campaign) {
                        $players_of_campaign = array_intersect($players_of_campaign, $players_of_main);
                    }
                    $this->program->sync_players($campaign->id, $players_of_campaign);

                    $this->program->detach_campaign_player($campaign->id);
                    if ($players_of_campaign) {
                        $items = array();
                        foreach ($players_of_campaign as $player) {
                            $items[] = array('player_id' => $player, 'campaign_id' => $campaign->id);
                        }
                        $this->program->saveManyCampaignPlayer($items);
                    }
                }
    ...
    public function do_publish()
    {
        ...
        $result = $this->program->do_publish_time_slot($playlist);

        $result['id'] = $id;
        //FIXME

        if ($result['code'] == 0) {
            $updates = array('published' => $this->config->item('playlist.status.published'));
            $this->program->update_playlist($updates, $id);
            $ret = $this->do_publish_extended_campaigns($playlist);
            if ($ret['code'] != 0) {
                $result = $ret;
            }
        }
    ...
    public function do_save_media_check()
    {
        ...
        if ($media_type == $this->config->item('media_type_rss')) {
            ...
            $count = $this->program->get_playlist_area_media_count($playlist_id, $area_id, $flag);
            if ($count > 0) {
                $code = 1;
    ...
    public function do_change_media_order()
    {
        ...
        if ($fid && $sid) {
            $this->load->model('program');
            if ($this->program->change_playlist_area_media_order($fid, $sid) === false) {
                $code = 1;
    ...
    public function do_move_to()
    {
        ...
        if ($id) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($this->program->playlist_area_media_move_to($id, $index)) {
    ...
    public function do_publish_extended_campaigns($main_campain)
    {
        $this->load->model('program');
        $extended_campaigns = $this->program->get_extended_campaigns($main_campain->id);
        ...
        if ($extended_campaigns) {
            ...
            foreach ($extended_campaigns as $campaign) {
                ...
                $this->program->fill_campaign_media_info($campaign);
                $campaign->tags =  $this->program->get_campaign_tags($campaign->id);

                $result = $this->program->do_publish_time_slot($campaign);
    ...
    public function do_refresh_all_campaigns()
    {
        ...
        */
        $campaigns = $this->program->get_published_campaign($cid);
        $this->load->model('membership');
        ...
        if ($campaigns && $campaigns['total'] > 0) {
            ...
            foreach ($campaigns['data'] as $campaign) {
                ...
                */
                ...
                if ($players) {
                    ...
                    $items = array();
                    foreach ($players as $player) {
                        $items[] = array('player_id' => $player->id, 'campaign_id' => $campaign->id);
                    }
                    $this->program->saveManyCampaignPlayer($items);
                }
            ...
            foreach ($campaigns['data'] as $campaign) {
                ...
                $this->program->update_playlist(array('published' => 0, 'update_time' => date('Y-m-d H:i:s')), $campaign->id);
                $result = $this->program->do_publish_time_slot($campaign);
            ...
            if ($capacitys) {
                ...
                foreach ($capacitys as $key => $capacity) {
                    ...
                    if ($play_type == 0) {
                        ...
                        $pl_used = floor(3600 * ($play_count * $capacity['quota'] / 100) / 100);
                    } elseif ($play_type == 2) {
    ...
    public function sync_campaign($id, $priority)
    {
    ...
    public function get_master_campgains()
    {
        $this->load->model('program');
        $cid = $this->get_cid();
        $data = $this->program->get_master_campaigns($cid);
        echo json_encode($data);
    }

    public function get_selected_campaign_data()
    {
        $cam_id = $this->input->get('cam_id');
        $this->load->model('program');
        $data = $this->program->get_selected_campaign($cam_id);
        echo json_encode($data);
    }
}
