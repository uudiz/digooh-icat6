<table class="table-list"  width="100%" >
      ...
      <th width="100">
			...
			<!--
			<th width="90">
				<a href="javascript:void(0);" onclick="mediaLib.page(<?php echo $curpage;?>,'add_user_id','<?php if($order_item == 'add_user_id' && $order == 'desc'):?>asc<?php else:?>desc<?php endif;?>');" ><?php echo lang('author');?></a>
      ...
      <th width="120">
				<a href="javascript:void(0);" onclick="mediaLib.page(<?php echo $curpage;?>,'add_time','<?php if($order_item == 'add_time' && $order == 'desc'):?>asc<?php else:?>desc<?php endif;?>');" ><?php echo lang('upload.time');?></a>
        ...
        <a href="javascript:void(0);" onclick="mediaLib.page(<?php echo $curpage;?>,'play_time','<?php if($order_item == 'play_time' && $order == 'desc'):?>asc<?php else:?>desc<?php endif;?>');" ><?php echo lang('playtime');?></a>
			...
			<th width="80">
				<a href="javascript:void(0);" onclick="mediaLib.page(<?php echo $curpage;?>,'start_date','<?php if($order_item == 'start_date' && $order == 'desc'):?>asc<?php else:?>desc<?php endif;?>');" ><?php echo lang('date.range');?></a>
		...
		<!-- 	
		 ...
		 <a href="javascript:void(0);" onclick="mediaLib.page(<?php echo $curpage;?>,'start_time','<?php if($order_item == 'start_time' && $order == 'desc'):?>asc<?php else:?>desc<?php endif;?>');" ><?php echo lang('time.range');?></a>
				<img alt="" src="/images/icons/<?php if($order_item == 'start_time' && $order == 'desc'):?>dir-down.gif<?php elseif($order_item == 'start_time' && $order == 'asc'):?>dir-up.gif<?php else:?>dir-blank.gif<?php endif;?>" />
		 </th>
		 -->	
		...
		?>
			...
			<tr <?php if($index%2 != 0):?>class="even" onmouseout="this.className='even'" <?php else:?>onmouseout="this.className=''"<?php endif;?>  onmouseover="this.className='onSelected'">
			  ...
			  <td>
			  	<?php if(mb_strlen($row->descr) > 32){echo mb_substr($row->descr, 0, 32).'...';}else{echo $row->descr;}?>
			  </td>
			  <!--
			  <td>
				<?php
			...
			<tr <?php if($index%2 != 0):?>class="even"<?php endif;?>>
			  ...
			  <td style="text-align:right;">
			  	...
			  	<!-- 
					<?php if($auth >= $ADMIN):?>
			  		<a href="javascript:void(0);" class="button" onclick="mediaLib.createFolder(this);" title="<?php echo lang('title.create.folder');?>"><?php echo lang('create');?></a>
					<?php endif;?>
				 -->	
			  		<a href="javascript:void(0);" class="button"  onclick="mediaLib.moveTo('<?php echo lang('tip.empty.media');?>','<?php echo lang('tip.empty.folder');?>',2);"><?php echo lang('move.to');?></a>
				</p>
