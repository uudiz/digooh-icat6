...
<h2 class="tit-01">
	...
	<div class="filter" style="float:right;margin:0px 16px;padding-right:2px;">
		...
		<select id="filterType" name="filterType" style="width: 100px;margin:0px 4px;" onchange="campaign.changeFilterType(this);">
		...
		<select id="filterFolder" name="filterFolder" style="width: 150px; margin:0px 4px;" onchange="campaign.addAreaMediaFilter(<?php echo $playlist_id.','.$area_id.',\''.$bmp.'\','.$media_type.',1';?>);">
			<option vlaue="0" selected><?php echo lang('all');?></option>
			<?php if ($auth==$ADMIN&&!$pid):?>
			<option value="0" ><?php echo lang('folder.default');?></option>
			<?php endif;?>
			<?php if (!$this->config->item('with_sub_folders')):?>	
			<?php if (isset($folders)&&is_array($folders)):?>
		...
		<a href="javascript:void(0);" class="btn-go" style="margin-left:20px;" onclick="campaign.addAreaMediaFilter(<?php echo $playlist_id.','.$area_id.',\''.$bmp.'\','.$media_type.',1';?>);"><label><?php echo lang('filter');?></label></a>
	</div>
</h2>
...
<script>
	...
	if(event.keyCode == 13){
		campaign.addAreaMediaFilter(<?php echo $playlist_id.','.$area_id.',\''.$bmp.'\','.$media_type.',1';?>);
	}
};
	campaign.initMediaPanel();
	<?php if ($this->config->item('with_sub_folders')):?>	
