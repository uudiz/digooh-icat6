<div class="page-name">
	<?php
    if ($this->config->item('covert.office.files')==true) {
        echo lang('media.upload.videos.office');
    } else {
        echo lang('media.upload.videos');
    }
    ?>
</div>
	...
	var settings = {
		...
		validation: {
						<?php
                            if ($this->config->item('covert.office.files')==true):
                        ?>
							allowedExtensions: ["mp4", "mkv", "mpg", "mpeg", "avi", "flv", "wmv", "divx", "mov","ppt","pptx","swf","pdf"],
						<?php else:?>				
							allowedExtensions: ["mp4", "mkv", "mpg", "mpeg", "avi", "flv", "wmv", "divx", "mov"],
						<?php endif; ?>
	        sizeLimit: 500000000
	       	//itemLimit: 5//单次最大上传数量
		},
		request: {
			endpoint: "/media/file_upload_videos"
		},
		retry: {
	  		enableAuto: true
		},
...
<script type="text/javascript">
	...
	$("#folder_select").select2ToTree({treeData: {dataArr: mydata}/*, maximumSelectionLength: 3*/});		
	<?php endif?>	
</script>
