...
<table cellspacing="0" cellpadding="0" border="0" class="from-panel">
		<tbody>
			<tr>
				...
				<td>
					<input type="text" id="name" name="name" style="width:150px;" value="<?php echo $tag->name;?>" />
				</td>
				<td>
			...
			<tr>
				...
				<td>
					<textarea name="descr" id="descr" rows="3" style="width:150px;"><?php echo $tag->descr;?></textarea>
				</td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				...
				<td>
					<select  data-placeholder="" id="players-select-options" class="chosen-select" multiple style="max-height:100px">
						...
						<?php foreach ($players as $player):?>
							<option value="<?php echo $player->id;?>" <?php if (isset($tag->players)&&is_array($tag->players)&&in_array($player->id, $tag->players)):?>selected<?php endif;?>><?php echo $player->name;?></option>
						<?php endforeach;?>
					</select> 
					<span  >
...
<p class="btn-center">
	<input type="hidden" id="id" name="id" value="<?php echo $tag->id;?>" />
